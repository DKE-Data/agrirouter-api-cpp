...
static void jsonToMessage(const std::string &input, google::protobuf::Message *message) {
...
static RequestEnvelope createRequestHeader(const std::string &messageId, int64_t seqNo, const std::string &messageType, Addressing addressing, const std::string &teamSetContextId) {
...
static RequestPayloadWrapper createRequestBody(Message *message) {
  // Check for validity
  if (!message) {
...
static std::string encodeRequest(Request request) {
...
static Request decodeRequest(std::string encoded) {
  Request request;
  ...
  google::protobuf::io::ArrayInputStream inputStream(
      reinterpret_cast<void*>(decodedMsg), decodedRequest.size());
...
static Response decodeResponse(std::string encoded) {
  Response response;
  ...
  // Conversion to a coded input stream
  google::protobuf::io::ArrayInputStream inputStream(
      reinterpret_cast<void*>(decodedMsg), decodedResponse.size());
  google::protobuf::io::CodedInputStream codedStream(&inputStream);
...
static std::list<Response> getResponsesFromMessage(std::string message) {
  ...
  for (int i = 0; i < cJSON_GetArraySize(root); i++) {
    ...
    Response response = decodeResponse(msg->valuestring);
...
#endif  // LIB_AGRIROUTERCLIENT_INC_AGRIROUTERMESSAGEUTILS_H_
